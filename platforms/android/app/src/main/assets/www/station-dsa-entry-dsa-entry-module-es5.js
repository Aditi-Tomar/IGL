...
var DsaEntryPageRoutingModule = /** @class */ (function () {
    function DsaEntryPageRoutingModule() {
    }
    ...
    return DsaEntryPageRoutingModule;
}());
...
var DsaEntryPageModule = /** @class */ (function () {
    function DsaEntryPageModule() {
    }
...
var DsaEntryPage = /** @class */ (function () {
    function DsaEntryPage(formBuilder, router, activatedroute, commonServices, menu, dp) {
    ...
    DsaEntryPage.prototype.ngOnInit = function () {
    ...
    DsaEntryPage.prototype.GetStationCompany = function () {
        var _this = this;
        this.commonServices.post('CommonGetData', { Flag: 'CompanyByStation', Id: this.selectedStation, Status: 1 }).subscribe(function (resp) {
            _this.CompanyName = JSON.parse(resp).Table[0].CompanyName;
            _this.CompanyId = JSON.parse(resp).Table[0].CompanyId;
    ...
    DsaEntryPage.prototype.CheckCompany = function () {
        if (this.CompanyId == '1' ||
    ...
    DsaEntryPage.prototype.openFirst = function () {
    ...
    DsaEntryPage.prototype.armACollapse = function () {
        if (this.flagAramA == 0) {
    ...
    DsaEntryPage.prototype.armBCollapse = function () {
        if (this.flagAramB == 0) {
    ...
    DsaEntryPage.prototype.riDateFetchStationEntry = function () {
        var _this = this;
        ...
        this.commonServices.post("CommonGetData", {
            ...
            if (data.Table[0].Msg == 'Allow') {
                _this.geteDate.selcteddate = _this.DPREntryDateTime;
                if (_this.dispenserList.length != 0) {
                    ...
                    _this.commonServices.presentToast('Dispenser Available');
                }
                else {
                ...
                _this.getSelectedDate(riDateStation[1]);
            }
        }, function (error) {
    ...
    DsaEntryPage.prototype.dsaEntryDetailRedirect = function () {
        var _this = this;
        var obj = {
            Id: this.selectedStation, Flag: 'validatePopup',
            Status: this.SummeryDate
        ...
        this.commonServices.post("CommonGetData", {
            ...
            if (data.Table[0].Msg == "Allow") {
                ...
                if (_this.dispenserList.length != 0) {
                    ...
                    //   });
                    // }
                    _this.router.navigate(['dsa-entry-detail']);
                }
                else {
                    _this.commonServices.presentToast('No Dispenser Available');
                }
    ...
    DsaEntryPage.prototype.getSelectedDate = function (datePicker) {
    ...
    DsaEntryPage.prototype.onStationSelect = function () {
        var _this = this;
        setTimeout(function () {
            _this.getSummaryHO();
        });
    };
    DsaEntryPage.prototype.FetchDSASubmittedData = function () {
        var _this = this;
        ...
        this.commonServices.FetchDSASubmittedData({ StationId: this.selectedStation,
            CDate: this.SummeryDate, StationCode: StationCode }).subscribe(function (resp) {
            var arr = [];
            ...
            if (data1 != '') {
                arr = data1.Table;
                console.log(arr);
                _this.popupDate = _this.dp.transform(_this.SummeryDate, 'dd-MMM-yyyy');
            }
        }, function (error) {
            _this.commonServices.presentToast('Something went wrong.');
        });
    };
    DsaEntryPage.prototype.GetBasicSummerData = function () {
        var _this = this;
        this.commonServices.loadingPresent();
        this.commonServices.post("CommonGetData", { Id: this.MoUserId, Flag: 'SummeryFlag' })
            .subscribe(function (resp) {
            _this.Cdate = JSON.parse(resp).Table2[0].Cdate;
            console.log(_this.Cdate, 'Cdate');
            _this.DepartmentCode = JSON.parse(resp).Table2[0].DepartmentCode;
            _this.SummeryDate = _this.Cdate;
            _this.RegionList = JSON.parse(resp).Table;
            _this.RegionName = _this.RegionList[0].RegionName;
            console.log(_this.DepartmentCode, "DepartmentCode");
            _this.selectedRegion = JSON.parse(resp).Table[0].RegionId;
            _this.StationList = JSON.parse(resp).Table1;
            console.log(_this.StationList, 'Sapcode');
            _this.currentStationName = _this.StationList[0].StationName;
            _this.SapCode = _this.StationList[0].SapCode;
            console.log(_this.currentStationName);
            _this.selectedStation = JSON.parse(resp).Table1[0].StationId;
            localStorage.setItem("StationId", JSON.stringify(_this.selectedStation));
            //this.FetchDSASubmittedData();
            _this.riDateFetchStationEntry();
            setTimeout(function () {
                _this.getSummaryHO();
                _this.GetStationCompany();
            });
            _this.commonServices.loadingDismiss();
        }, function (error) {
            _this.commonServices.presentToast('Something went wrong.');
    ...
    DsaEntryPage.prototype.onRegionSelect = function () {
        var _this = this;
        //this.commonServices.loadingPresent();
        this.commonServices.post("CommonGetData", {
            ...
            _this.StationList = JSON.parse(resp).Table;
            if (JSON.parse(resp).Table != undefined) {
                _this.selectedStation = JSON.parse(resp).Table[0].StationId;
            }
            //   setTimeout(() => {
            _this.getSummaryHO();
            //   });
    ...
    DsaEntryPage.prototype.getSummaryHO = function () {
        var _this = this;
        this.sideBarIsOpened = false;
        // this.commonServices.loadingPresent();
        this.commonServices.post("getSummaryHO", { StationId: this.selectedStation,
            EntryDate: this.SummeryDate }).subscribe(function (resp) {
            _this.commonServices.loadingDismiss();
            _this.dispenserList = JSON.parse(resp).Table;
            _this.Summary = JSON.parse(resp).Table1;
            //this.LockUnlock = ((JSON.parse(resp).Table2[0].LockFlag == 0)?true:false);
            _this.FetchDSASubmittedData();
            // this.riDateFetchStationEntry();
            _this.MySummaryDate = JSON.parse(resp).Table2[0].SummaryDate;
            //Lower detail for Left side
            _this.lowerDetLeft = JSON.parse(resp).Table3;
            //Lower detail for center side
            //this.RateDetails = JSON.parse(resp).Table2[0];
            _this.ForRate = JSON.parse(resp).Table2[0].Rate;
            _this.ForDiscountedRate = JSON.parse(resp).Table2[0].DisountedRate;
            _this.DisNonDicSmmry = JSON.parse(resp).Table4;
            //  this.commonServices.loadingDismiss();
        }, function (error) {
            _this.commonServices.presentToast('Something went wrong.');
            // this.commonServices.loadingDismiss();
    ...
    DsaEntryPage.prototype.getReadingByDispName = function (DispName, SumRow, DispSide) {
        for (var i = 0; i < Object.keys(SumRow).length; i++) {
    ...
    DsaEntryPage.ctorParameters = function () { return [
...
//# sourceMappingURL=station-dsa-entry-dsa-entry-module-es5.js.map