...
var JumpReadingSystemPageRoutingModule = /** @class */ (function () {
    function JumpReadingSystemPageRoutingModule() {
    }
    ...
    return JumpReadingSystemPageRoutingModule;
}());
...
var JumpReadingSystemPageModule = /** @class */ (function () {
    function JumpReadingSystemPageModule() {
    }
    ...
    return JumpReadingSystemPageModule;
}());
...
var JumpReadingSystemPage = /** @class */ (function () {
    function JumpReadingSystemPage(menu, router, activatedroute, commonServices, platform, fileOpener, datepipe, document, file, transfer, alertController) {
        this.menu = menu;
        this.router = router;
    ...
    JumpReadingSystemPage.prototype.ngOnInit = function () {
        var _this = this;
        var dt = new Date();
        ...
        setTimeout(function () {
            _this.getJumpReadingData();
        });
    };
    JumpReadingSystemPage.prototype.OnDateChnageFrom = function (val) {
        var self = this;
        var dt = new Date(val);
    ...
    JumpReadingSystemPage.prototype.OnDateChnageTo = function (val) {
        var self = this;
        var dt = new Date(val);
        //IOS Comment
    ...
    JumpReadingSystemPage.prototype.openFirst = function () {
    ...
    JumpReadingSystemPage.prototype.searchCollapse = function () {
    ...
    JumpReadingSystemPage.prototype.hideSearchBar = function () {
        this.searchShow = false;
        this.searchFlag = 0;
    };
    // get Date
    ...
    //   //datePicker.open();
    // }
    JumpReadingSystemPage.prototype.getJumpReadingData = function () {
        var _this = this;
        ...
        this.commonServices.post("CommonGetData", {
            ...
            if (JSON.parse(res).Table.length > 0) {
                _this.JumpReadingData = JSON.parse(res).Table;
                console.log(_this.JumpReadingData, "JumpReadingSystemDataByRole");
            }
            else {
                _this.JumpReadingData = [];
                _this.commonServices.presentToast("No Data Found.");
            }
        }, function (error) {
    ...
    JumpReadingSystemPage.prototype.OnDateChnage = function (val) {
        // const dt = new Date(val);
        // this.CDate= dt.getDate() + "/" + this.monthNames[dt.getMonth()] + "/" + dt.getFullYear();
        this.getJumpReadingData();
    };
    JumpReadingSystemPage.prototype.GetRowdata = function (itm) {
        this.Id = itm.ID,
    ...
    JumpReadingSystemPage.prototype.InsertRemarkByInstrumental = function () {
        var _this = this;
        var Json = {
            Id: this.Id,
        ...
        this.commonServices.loadingPresent();
        this.commonServices.post("InsertRemarkByInstrumental", Json).subscribe(function (res) {
            var data = JSON.parse(res);
            console.log(data.Table[0].Mesage, "Success");
            _this.commonServices.presentToast(data.Table[0].Mesage);
            _this.commonServices.loadingDismiss();
            // $('.modal').modal('hide');
            _this.getJumpReadingData();
        }, function (error) {
    ...
    JumpReadingSystemPage.prototype.UpdateJumpReadingStatusByRole = function (status, itm) {
        var _this = this;
        this.Action = status;
        ...
        if (this.ValidateJRS()) {
            this.commonServices.loadingPresent();
            this.commonServices.post("UpdateJumpReadingStatusByRole", Json).subscribe(function (res) {
                var data = JSON.parse(res);
                console.log(data.Table[0].Mesage, "Success");
                _this.commonServices.loadingDismiss();
                if (_this.DepartmentCode == 'SO' && status == 'A') {
                    _this.confirmCertificate(itm.ApprovedDate);
                }
                if (_this.DepartmentCode == 'HO' && status == 'A') {
                    _this.confirmCertificate(itm.ApprovedDate);
                }
                _this.commonServices.presentToast(data.Table[0].Mesage);
                _this.getJumpReadingData();
    ...
    JumpReadingSystemPage.prototype.confirmCertificate = function (ApprovedDate) {
                            ...
                            buttons: [
                                {
                                    ...
                                    text: 'OK',
                                    handler: function () {
                                        _this.GenerateJumpCertificate(ApprovedDate);
    ...
    JumpReadingSystemPage.prototype.GenerateJumpCertificate = function (ApprovedDate) {
        var _this = this;
        this.commonServices.loadingPresent();
        this.commonServices.post("JumpReadingCertificate", { Flag: 'getJumPReadingById', CDashdate: ApprovedDate, Id: this.Id, Status: this.StationId }).subscribe(function (res) {
            var data = JSON.parse(res);
            if (data) {
                var PdfUrl = "";
    ...
    JumpReadingSystemPage.prototype.goToJumpReamrk = function (value, itm) {
        this.GetRowdata(itm);
    ...
    JumpReadingSystemPage.prototype.ValidateJRS = function () {
    ...
    JumpReadingSystemPage.ctorParameters = function () { return [
...
//# sourceMappingURL=HO-jump-reading-system-jump-reading-system-module-es5.js.map